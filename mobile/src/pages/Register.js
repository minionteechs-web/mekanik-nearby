...
const createStyles = (colors) => ({
    ...
    mechanicHeaderTitle: { fontSize: 14, fontWeight: '800', color: colors.textMain, marginBottom: 2 },
    mechanicHeaderSub: { fontSize: 12, color: colors.textMuted },
    mechanicDivider: {
        borderBottomWidth: 1,
...
function MechanicFields({ form, setField, styles, colors }) {
    return (
        <View style={styles.mechanicDivider}>
            <View style={styles.mechanicHeader}>
                <Wrench size={18} color={colors.brand} />
                <View style={{ flex: 1 }}>
                    <Text style={styles.mechanicHeaderTitle}>Workshop details</Text>
                    <Text style={styles.mechanicHeaderSub}>Required to join as a mechanic</Text>
                </View>
            </View>
            <Input
...
export const Register = ({ navigation }) => {
    ...
    const handleRegister = async () => {
        if (!acceptedTerms) {
            return Alert.alert('Terms required', 'Please accept the Terms of Service and Privacy Policy to continue.');
        }

        if (form.role === 'mechanic') {
            if (!form.workshopName.trim() || !form.specialty || !form.city.trim()) {
                return Alert.alert(
                    'Workshop details required',
                    'Workshop name, specialty, and city are required to join as a mechanic.'
                );
            }
        }
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
                ...
                {form.role === 'mechanic' && (
                    <MechanicFields form={form} setField={setField} styles={styles} colors={colors} />
                )}
                ...
                <Button onPress={handleRegister} disabled={loading || !acceptedTerms} style={styles.registerBtn}>
